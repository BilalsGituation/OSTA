...
Although the `AnnData` class comes from Python, it has been ported to R via the previously-mentioned `anndataR` package.
...
We can use `anndataR` to access `AnnData` object variables using R syntax. For basic outputs, this works out of the box:
...
`anndataR` also supports a few direct type conversions (e.g. `SingleCellExperiment` $\leftrightarrow$ `AnnData` $\leftrightarrow$ `Seurat`). In the example demonstrated here, we convert from `AnnData` to `SingleCellExperiment`:

```{r conv2sce, message=FALSE}
sce <- ad$as_SingleCellExperiment()
```

With `anndataR`, we can also read the `.h5ad` file in as a`SingleCellExperiment`.
...
Here we use Python's `anndata` module instead of R package `anndataR` to read in the `exported_h5ad` object.
